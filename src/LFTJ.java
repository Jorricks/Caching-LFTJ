...
public class LFTJ {
    ...
    private ArrayList<ArrayList<RelationIterator<Integer>>> iteratorPerDepth; // Contains the iterator for each dept
    private ArrayList<Integer> currentTuple = new ArrayList<>(); // Contains the key values of all parents
    private int p = 0; // Current iterator pointer
    private int numIters; // Number of iterators at this depth
    private int depth = -1; // Current depth
    ...
    private LFTJ() throws IOException {
        ...
        initDataSets("./data/self-created-test.txt", CycleOrRoundsEnum.ROUNDS, 3 );
    ...
    private void initDataSets(String fileName, Enum CycleOrRounds, int amountOfRelations) throws IOException{
        ...
        for(i = 1; i <= amountOfRelations; i++){
            DataImporter di = new DataImporter(fileName);
